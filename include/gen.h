...
#define BASIC(a,b,c)case a:{\
        ...
        BASIC('%', FRem, SRem)
        case '/': {
            if (type == llvm::Type::FloatTyID || type == llvm::Type::DoubleTyID) return gen::builder.CreateFDiv(
                lhs, rhs, "FDiv""_tmp");
            if (type == llvm::Type::IntegerTyID)
...
#define CMP(a,b,c)case a:\
        ...
        case '=': {
            if (lhs->getType()->getTypeID() != llvm::Type::PointerTyID)return gen::LogErrorV(
