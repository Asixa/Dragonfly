...
namespace gen {
	...
	static std::unique_ptr<llvm::Module> the_module = std::make_unique<llvm::Module>("Program", gen::the_context);
	static llvm::IRBuilder<> builder(gen::the_context);
	static llvm::Function* the_function;
	static std::map<std::string, llvm::Value*> named_values;
	static std::map<std::string, parser::ClassDecl*> named_types;

	static llvm::Value* True = llvm::ConstantInt::get(llvm::IntegerType::get(gen::the_context, 1), 1);
	static llvm::Value* False = llvm::ConstantInt::get(llvm::IntegerType::get(gen::the_context, 1), 0);
	...
	namespace parser
	{
		...
		inline llvm::Value* Field::Gen(int cmd)
		{
			...
			if (names.size() > 1)
			{
				...
				{
					auto name = names[i];
					auto decl = gen::named_types[v->getType()->getPointerElementType()->getPointerElementType()->getStructName().str()];
					auto idx = -1;
...
#define BASIC(a,b,c)case a:{\
			...
			case '/':
				{
					if (type == llvm::Type::FloatTyID || type == llvm::Type::DoubleTyID) return gen::builder.CreateFDiv(lhs, rhs, "FDiv""_tmp");
					if (type == llvm::Type::IntegerTyID)
...
#define CMP(a,b,c)case a:\
		{\
			...
			case '=': {
				if (lhs->getType()->getTypeID() != llvm::Type::PointerTyID)return gen::LogErrorV("cannot reassign a constant\n");
				auto rhv = rhs;
				if (rhs->getType()->getTypeID() != lhs->getType()->getPointerElementType()->getTypeID())
					rhv = rhs->getType()->getTypeID() == llvm::Type::PointerTyID ? gen::AlignLoad(gen::builder.CreateLoad(rhs)) : rhs;
...
#define BASIC_ASSGIN(a,b,c,d)case a:																					\
			...
			case DivAgn:
			{
				...
				if (type == llvm::Type::PointerTyID) {
					...
					const auto lhsv = gen::AlignLoad(gen::builder.CreateLoad(lhs));
					if (type == llvm::Type::FloatTyID || type == llvm::Type::DoubleTyID)
						return gen::AlignStore(gen::builder.CreateStore(gen::builder.CreateFDiv(lhsv, rhv, "FDiv""_tmp"), lhs));
					if (type == llvm::Type::IntegerTyID)
					{
						const auto lhv_d = gen::builder.CreateCast(llvm::Instruction::SIToFP, lhsv, llvm::Type::getDoubleTy(gen::the_context));
						const auto rhv_d = gen::builder.CreateCast(llvm::Instruction::SIToFP, rhv, llvm::Type::getDoubleTy(gen::the_context));
						const auto div = gen::builder.CreateFDiv(lhv_d, rhv_d, "div_tmp");
						const auto div_i = gen::builder.CreateCast(llvm::Instruction::FPToUI, div, llvm::Type::getInt32Ty(gen::the_context));
		...
		inline void FunctionDecl::GenHeader()
		{
			...
			if (!the_function)
			{
				...
				the_function = llvm::Function::Create(func_type, llvm::Function::ExternalLinkage, gen::WstrToStr(name), gen::the_module.get());
		...
		inline void ClassDecl::Gen()
		{
			...
			auto function = llvm::Function::Create(func_type, llvm::Function::ExternalLinkage, gen::WstrToStr(name), gen::the_module.get());
			const auto bb = llvm::BasicBlock::Create(gen::the_context, gen::WstrToStr(name) + "_entry", function);
		...
		{
            gen::BuildInFunc("malloc", llvm::Type::getInt8PtrTy(gen::the_context), std::vector<llvm::Type*>{ llvm::Type::getInt32Ty(gen::the_context) });
            gen::BuildInFunc("free", llvm::Type::getVoidTy(gen::the_context), std::vector<llvm::Type*>{ llvm::Type::getInt8PtrTy(gen::the_context) });
            gen::BuildInFunc("printf", llvm::Type::getVoidTy(gen::the_context), std::vector<llvm::Type*>{ llvm::Type::getInt8PtrTy(gen::the_context) }, true);

			for (auto& declaration : declarations)declaration->GenHeader();
			for (auto& declaration : declarations)declaration->Gen();

			auto main_func = gen::CreateFunc(gen::builder, "main");
